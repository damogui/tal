...
public class ResponserManager {
    private static Log                    logger            = LogFactory.getLog("weixin."+ResponserManager.class.getSimpleName());
    private static List<IWeixinResponsor> weixinResponseors = new ArrayList<IWeixinResponsor>();

    private static ReentrantLock lock = new ReentrantLock();

    public static List<IWeixinResponsor> getResponseors() throws NetsharpException {
    ...
    public static void reload() throws NetsharpException {
        try {
            lock.lock();
            if (weixinResponseors.size() > 0) {
                return;
