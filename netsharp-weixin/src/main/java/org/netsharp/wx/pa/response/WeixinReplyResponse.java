...
//	private static Log logger = LogFactory.getLog("weixin."+WeixinReplyResponse.class.getSimpleName());
	...
	private NReply getReply(RequestMessage request) {
		...
		if (request instanceof TextRequest) {
			...
			reply = (NReply)pam.getReply(text.getContent(), origianlId);
	...
	public final ResponseMessage response(RequestMessage request){
		NReply reply = (NReply) localReply.get();
