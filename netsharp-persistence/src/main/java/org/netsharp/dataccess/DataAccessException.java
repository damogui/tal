...
public class DataAccessException extends NetsharpException {
	...
	private static final Log logger = LogFactory.getLog("dao."+DataAccessException.class.getName());
	...
	public DataAccessException(String name,String cmdText,QueryParameters qps, Throwable throwable){
		...
		logger.error( StringManager.NewLine + "数据库访问异常，名称："+name);
		logger.error( StringManager.NewLine + cmdText);
