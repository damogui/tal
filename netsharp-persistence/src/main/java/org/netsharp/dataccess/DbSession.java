...
public class DbSession {
	...
	private static final Log conLogger = LogFactory.getLog("dao."+DbSession.class.getName());
	private static final Log transactionLogger = LogFactory.getLog("transaction."+DbSession.class.getName());
	...
	public void commit() {
		try {
			if (con!=null){
				transactionLogger.debug("} commit");
				con.commit();
	...
	Connection getConnection() {
		...
		try {
			...
			String message=e.getMessage()+StringManager.NewLine
					+e.getClass().getName()+StringManager.NewLine
					+"未能创建数据库连接,url:"+dbp.getJdbcUrl()+";username:"+dbp.getUsername()+";password:"+dbp.getPassword();
	...
	public void setTransaction(boolean isTransaction) throws SQLException {
		...
		if(isTransaction){
			transactionLogger.debug("isTransaction:"+isTransaction);
		}
