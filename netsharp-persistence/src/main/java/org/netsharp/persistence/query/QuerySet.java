...
import org.netsharp.persistence.TTable;
...
public class QuerySet extends AbstractQuery {
	...
	@Override
	protected TSet queryOnece() throws NetsharpException {
	...
	@Override
	protected TSet queryPaging() throws NetsharpException {
	...
	private TSet doQuery(OqlContex context) throws NetsharpException {
		// -----------------------------------------------------
		// 分解OQL解析顺序
		// -----------------------------------------------------
	...
	private void query(OqlStructs oss) throws NetsharpException {
		...
		builder.append("SELECT ").append(StringManager.join(",", selectes)).append(StringManager.NewLine)
				.append("FROM ").append(os.Mtable.getTableName());
 		...
 		if (os.IsMain) {
			...
			for (OqlStruct oqlStruct : ss.values()) {
				...
				if (parentTable != null) {
					...
					filters.add(oqlStruct.Mtable.getPropertyOrColumn( oqlStruct.OutInId).getColumnName() + " IN (" + filter + ")");
				} else {
