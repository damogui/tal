...
import org.netsharp.persistence.PersistUtil;
...
public abstract class AbstractSqlGenerator implements ISqlGenerator {
	...
	protected SqlStruct generateDeleteSql(Mtable meta) {
		...
		if (PersistUtil.isConcurrency(meta)) {

			whereString.add("Ts=?");
			paramField.add(new ParamField("@Ts", "Ts"));
		}
	...
	private SqlStruct generateUpdateSql(Mtable meta, String[] properties) {
		...
		if (PersistUtil.isConcurrency(meta)) {
			keys.add("Ts=?");
			paramField.add(new ParamField(prefix + "Ts", "Ts"));
		}
