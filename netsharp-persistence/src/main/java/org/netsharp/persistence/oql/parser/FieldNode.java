...
public class FieldNode
{
	...
	public void join(ArrayList<String> joins)
    {
        ...
        else if (OrmType == OrmType.Reference || OrmType == OrmType.CompositeOne)
        {
            ...
            String join = "LEFT JOIN {ReferenceTableName} AS {ReferenceRole} " + Oql.WithNolock() + " ON {MainRole}.{FK}={ReferenceRole}.Id";// + Oql.WithNolock;
        ...
        else if (OrmType == OrmType.Subs)
        {
            ...
            String join = "LEFT JOIN {SubTableName} AS {SubRole} " + Oql.WithNolock() + "ON {SubRole}.{FK}={MainRole}.Id";
