...
public class DepartmentMonthHandler extends AbstractReportHandler{
	...
	protected AbstractReportHandler getNextHandler() {
		...
		return new DepartmentYearHandler();
	}
	...
	private void createParentDepartment(List<Integer> departmentIdList) {
		...
		for (Integer parentId : parentIdList) {
			...
			SelectBuilder builder = SelectBuilder.getInstance();
			{
				...
				builder.where("year=?", "month=?","date_type=?", "organization_type=?", "department_id in (" + childIds + ")");
			}
	...
	public void updateParentId(Integer parentId, String childDepartmentIds) {
		...
		UpdateBuilder updateBuilder = new UpdateBuilder();
		{
			...
			updateBuilder.where("year=?", "month=?","date_type=?", "organization_type=?", "department_id in (" + childDepartmentIds + ")");
		}
	...
	public void updateParentId(FranchiseeReport entity) {
		...
		UpdateBuilder updateBuilder = new UpdateBuilder();
		{
			...
			updateBuilder.where("year=?", "month=?", "date_type=?","organization_type=?","department_id=?");
		}
