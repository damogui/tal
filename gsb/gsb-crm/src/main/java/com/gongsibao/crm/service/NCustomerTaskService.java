...
public class NCustomerTaskService extends SupplierPersistableService<NCustomerTask> implements INCustomerTaskService {
    ...
    public Boolean batchAllocation(String[] taskIdArray, Integer supplierId, Integer departmentId, Integer toUserId) {
        boolean isNotify = false;
        List<Integer> taskIdList = Arrays.asList(taskIdArray);
        List<NCustomerTask> taskList = getByIdList(taskIdList);
        if (CollectionUtils.isEmpty(taskList)) {
            return false;
        }
        ActionContext ctx = getAllocationTaskAction(taskList, taskList.size(), isNotify, supplierId, departmentId, toUserId);
        ActionManager action = new ActionManager();
        action.execute(ctx);
        return true;
    }
    ...
    private ActionContext getAllocationTaskAction(List<NCustomerTask> taskList, int alloCount, Boolean isNotify, Integer supplierId, Integer departmentId, Integer toUserId) {
        // 商机分配
        Map<String, Object> setMap = new HashMap<String, Object>();
        if (NumberUtils.toInt(toUserId) != 0) {
            Salesman salesman = salesmanService.byEmployeeId(toUserId);
            if (salesman != null) {
                supplierId = salesman.getSupplierId();
    ...
    @Override
    public NCustomerTask getById(Integer id) {
        Oql oql = new Oql();
        {
            oql.setType(this.type);
    ...
    @Override
    public List<NCustomerTask> getByIdList(List<Integer> idList) {
