...
public class DepartmentProductDetailPart extends TaskProductDetailPart{
	...
	public List<Dict> queryProductFirstCategory(Integer departmentId,Integer supplierId) {
		...
		if(departmentId == null||departmentId.equals(0)){
			...
			oql.setFilter("type=201 and parentId=0 and enabled=1 and id in (select product_category_id_1 from sp_service_product where id=?)");
			oql.getParameters().add("supplierId", supplierId, Types.INTEGER);
		}else{

			oql.setFilter("type=201 and parentId=0 and enabled=1 and id in (select product_category_id_1 from Depart_service_product where department_id=?)");
	...
	public List<Dict> queryProductSecondCategory(Integer departmentId,Integer supplierId,Integer parentId) {
		...
		if(departmentId == null||departmentId.equals(0)){
			...
			oql.setFilter("parentId=? and type=201 and enabled=1 and id in (select product_category_id_2 from sp_service_product where id=?)");
	...
	public List<Dict> queryProvince(Integer departmentId,Integer supplierId){
		...
		if(departmentId == null||departmentId.equals(0)){
			...
			oql.setFilter("type=101 and parentId=0 and enabled=1 and id in (select province_id from sp_service_product where id=?)");
			oql.getParameters().add("supplierId", supplierId, Types.INTEGER);
		}else{

			oql.setFilter("type=101 and parentId=0 and enabled=1 and id in (select province_id from Depart_service_product where department_id=?)");
			oql.getParameters().add("departmentId", departmentId, Types.INTEGER);
		}
	...
	public List<Dict> queryCity(Integer departmentId,Integer supplierId,Integer provinceId){
		...
		if(departmentId == null||departmentId.equals(0)){
			...
			oql.setFilter("type=101 and enabled=1 and id in (select city_id from sp_service_product where id=?)");
			oql.getParameters().add("supplierId", supplierId, Types.INTEGER);
		}else{

			oql.setFilter("type=101 and enabled=1 and id in (select city_id from Depart_service_product where department_id=?)");
			oql.getParameters().add("departmentId", departmentId, Types.INTEGER);
		}
	...
	public List<Dict> queryCounty(Integer departmentId,Integer supplierId,Integer cityId){
		...
		if(departmentId == null||departmentId.equals(0)){
			...
			oql.setFilter("type=101 and enabled=1 and id in (select county_id from sp_service_product where id=?)");
			oql.getParameters().add("supplierId", supplierId, Types.INTEGER);
		}else{

			oql.setFilter("type=101 and enabled=1 and id in (select county_id from Depart_service_product where department_id=?)");
			oql.getParameters().add("departmentId", departmentId, Types.INTEGER);
		}
