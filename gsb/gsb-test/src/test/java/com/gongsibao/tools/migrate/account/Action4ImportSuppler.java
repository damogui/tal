...
public class Action4ImportSuppler extends AbstractActionService {
	...
	public void run() {
		...
		String cmdText = "INSERT INTO sp_supplier (contact,admin_id,mobile_phone,type, qq, weixin, email, sex, NAME, head_portrait, create_time, creator_id, identity_card, identity_card_photo_front, identity_card_photo_reverse, response_time, service_declaration, service_intro, consignee_address, bank_account_type, bank_name, bank_num, bank_account_name, STATUS ) SELECT NAME,uc_user_id, mobile_phone, 2 AS type, qq, weixin, email, sex, company_name, head_thumb_file_id, add_time, add_user_id, id_card, id_card_front_file_id, id_card_back_file_id, ( CASE response_time WHEN '5分钟内' THEN '5' WHEN '10分钟内' THEN '10' WHEN '20分钟内' THEN '20' WHEN '30分钟内' THEN '30' WHEN '1小时内' THEN '60' WHEN '4小时内' THEN '240' WHEN '8小时内' THEN '480' ELSE NULL END ) AS response_time, declar, intro, receive_address, ( CASE bank_type WHEN '单位帐户' THEN 1 WHEN '个人帐户' THEN 2 ELSE NULL END ) AS bank_type, bank_account, bank_no, bank_account, ( CASE is_enabled WHEN 0 THEN 3 WHEN 1 THEN 1 ELSE NULL END ) AS is_enabled FROM uc_suppler; ";
