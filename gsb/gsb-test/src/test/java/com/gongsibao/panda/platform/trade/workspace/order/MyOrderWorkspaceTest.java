...
public class MyOrderWorkspaceTest  extends WorkspaceCreationBase{
	...
	protected PDatagrid createDatagrid(ResourceNode node) {
		...
		column = addColumn(datagrid, "payStatus", "支付状态", ControlTypes.ENUM_BOX, 90);
		column = addColumn(datagrid, "processStatus", "执行进度", ControlTypes.ENUM_BOX, 90);
		column = addColumn(datagrid, "refundStatus", "退款状态", ControlTypes.ENUM_BOX, 90);		
		...
		column = addColumn(datagrid, "platformSource", "订单来源", ControlTypes.ENUM_BOX, 90);
		addColumn(datagrid, "payTime", "支付时间", ControlTypes.DATE_BOX, 130);
		addColumn(datagrid, "createTime", "创建时间", ControlTypes.DATE_BOX, 130);
		column = addColumn(datagrid, "type", "订单类型", ControlTypes.ENUM_BOX, 100, true);
		column = addColumn(datagrid, "prodName", "服务名称", ControlTypes.TEXT_BOX, 100);
		{
			column.setFormatter("return '<span title='+value+'>'+value+'</span>'");
		}
		column = addColumn(datagrid, "sourceType", "来源类型", ControlTypes.ENUM_BOX, 90);
	...
	protected PQueryProject createQueryProject(ResourceNode node) {
		...
		addQueryItem(queryProject, "changePriceAuditStatus", "改价申请订单", ControlTypes.ENUM_BOX);
		addQueryItem(queryProject, "refundStatus", "退单申请订单", ControlTypes.ENUM_BOX);
