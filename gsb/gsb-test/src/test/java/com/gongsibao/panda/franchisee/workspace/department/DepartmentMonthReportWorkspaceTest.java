...
public class DepartmentMonthReportWorkspaceTest extends WorkspaceCreationBase {
	...
	public void setup() {
		...
		listFilter = "date_type=3 AND (FranchiseeReport.department_id IN ({departments}) or department_id in (SELECT parent_id from sys_permission_organization where id in ({departments})))";
		// 扩展
	...
	protected PDatagrid createDatagrid(ResourceNode node) {
		...
		column = addColumn(datagrid, "month", "月份", ControlTypes.NUMBER_BOX, 80, true);
		{
			column.setAlign(DatagridAlign.CENTER);
		}
