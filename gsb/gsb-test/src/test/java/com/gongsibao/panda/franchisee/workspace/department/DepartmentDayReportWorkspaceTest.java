...
public class DepartmentDayReportWorkspaceTest extends WorkspaceCreationBase {
	...
	public void setup() {
		...
		listFilter = "date_type=5 AND (FranchiseeReport.department_id IN ({departments}) or department_id in (SELECT parent_id from sys_permission_organization where id in ({departments})))";
		// 扩展
