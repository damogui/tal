...
public class DictWorkspaceTest extends WorkspaceCreationBase  {
	...
	public void setup() {
		...
		urlList = "/basic/dict/manage/list";//列表的url
		urlForm = "/basic/dict/manage/form";//弹出框的url
		listPartName = formPartName = "字典信息";
		meta = MtableManager.getMtable(entity);//获取实体元数据
		formPartName = listPartName = meta.getName();
		resourceNodeCode = "GSB_Basic_Dict_" + Dict.class.getSimpleName();//菜单节点码（名称）
	...
	protected PDatagrid createDatagrid(ResourceNode node) {
		...
		addColumn(datagrid, "id", "主键", ControlTypes.TEXT_BOX, 100);
	...
	protected PQueryProject createQueryProject(ResourceNode node) {
		...
		//addQueryItem(queryProject, "pid", "类型", ControlTypes.);
		return queryProject;
	}
	//默认的表单配置信息
	protected PForm createForm(ResourceNode node) {
		...
		//addFormField(form, "id", "主键", null, ControlTypes.NUMBER_BOX, true, false);		
