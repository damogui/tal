...
public class RoleTest {
	...
	IRoleWorkbenchService workbenchService = ServiceFactory.create(IRoleWorkbenchService.class);
	...
	public void run() {
		...
		this.create("Platform_Finance_PTYG", "财务_普通员工", "Platform_Finance", workbench);
		...
		// 2.服务商
		workbench = createWorkbench("服务商管理系统","/panda/supplier/workbench");
		...
		this.create("Supplier_Order_Operator_Leader", "订单_操作组长", "Supplier_Order", workbench);
	...
	private RoleWorkbench createWorkbench(String name, String path) {
		...
		RoleWorkbench workbench = byPath(path);
		if (workbench == null) {

			workbench = new RoleWorkbench();
			{
				workbench.toNew();
				workbench.setName(name);
				workbench.setPath(path);
			}
			workbench = workbenchService.save(workbench);
		}

		return workbench;
	}

	private RoleWorkbench byPath(String path) {
		...
		RoleWorkbench workbench = workbenchService.queryFirst(oql);
		return workbench;
	}

	private void create(String code, String name, String groupCode, RoleWorkbench workbench) {
		...
		entity.setCode(code);
		entity.setName(name);
		entity.setWorkbenchId(workbench.getId());
