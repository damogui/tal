...
public class ActionAuditPerformanceWriteBack implements IAction{
    ...
    public void execute(ActionContext ctx) {
        ...
        Pay pay = (Pay) objectMap.get("nDepReceivableService");
        //审核
    ...
    private void audit(AuditState state, AuditLog auditLog, Pay pay, String remark) {
        switch (state.getValue()) {
            case 0://驳回审核
                ...
                nDepReceivableService.updateStatus(pay.getId(), AuditStatusType.Bhsh);
                break;
            case 1://通过审核
                auditService.auditApproved(auditLog.getId());
                if (auditLog.getLevel().equals(auditLog.getMaxLevel())) {
                    nDepReceivableService.updateStatus(pay.getId(), AuditStatusType.Shtg);
                }
                break;
        }
    }
