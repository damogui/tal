...
// 授权管理
//------------------------------------------------------------------------------------------------------------------------------------------
org.netsharp.organization.controller.position.OperationPositionPart = org.netsharp.panda.commerce.TreegridPart.Extends({
    ...
    initData: function () {
        ...
        var me = this;
        this.invokeService("authorizationPrincipal", [this.authorizationPrincipalId,4], function (data) {
    ...
    databindAuthorizationPrincipal: function () {
        ...
        $("i[collected=true]").each(function (index, image) {
            ...
            if (System.isnull (principalOperation )) {
                ...
                $(image).attr("clsss", OperationTypeColumn.No);
            }
            else {
                $(image).attr("clsss", OperationTypeColumn.Yes);
            }
        });
    ...
    onImageClicked: function (image) {
        ...
        var pars = [this.authorizationPrincipalId,4, $(image).attr("operationId"), operationType];
    ...
    onCheckboxClicked: function (checkbox) {
        ...
        var pars = [this.authorizationPrincipalId,4, ids.join("_"), operationType];
        this.invokeService("updatePrincipalOperation", pars, function (data) {
...
//
//org.netsharp.panda.commerce.OrganizationTreePart =  org.netsharp.panda.commerce.TreePart.Extends( {
//
//});