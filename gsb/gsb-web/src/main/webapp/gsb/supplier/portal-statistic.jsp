        	...
        	<div class="cell cell-12">
	        	<div class="easyui-panel" title="销售简报" data-options="fit:true,border:false">
	        		<p>
	        			...
	        			<span>未启动任务数：<%=homeEntity.getUnStartTasksCount()%>个</span>&nbsp;&nbsp;&nbsp;
	        			<span>待跟进任务数：<%=homeEntity.getUnfoolowTasksCount()%>个</span>&nbsp;&nbsp;&nbsp;
	        			<span>超时任务数：<%=homeEntity.getTimeOutTasksCount()%>个</span>&nbsp;&nbsp;&nbsp;
	        		</p>
	        		<p>
	        			<span>异常待跟进任务数：<%=homeEntity.getExceptUntreatedTasksCount()%>个</span>&nbsp;&nbsp;&nbsp;
	        			<span>公海：<%=homeEntity.getHighSeasCount()%>个</span>&nbsp;&nbsp;&nbsp;
	        		</p>
			    </div>
        	</div>	
        </div>
        	...
        	<div class="cell cell-12">
	        	<div class="easyui-panel" title="漏斗统计" data-options="fit:true,border:false">
	        		...
	        		<p>
	        			...
	        			<span style="padding-left: 15px;">A1:<%=homeEntity.getA1Count()%>个</span>
	        			<span style="padding-left: 15px;">A2:<%=homeEntity.getA2Count()%>个</span>
	        			<span style="padding-left: 15px;">A3:<%=homeEntity.getA3Count()%>个</span>
	        			<span style="padding-left: 15px;">A4:<%=homeEntity.getA4Count()%>个</span>
	        			<span style="padding-left: 15px;">B类：&nbsp;B1:<%=homeEntity.getB1Count()%>个</span>
	        			<span style="padding-left: 15px;">B2:<%=homeEntity.getB2Count()%>个</span>
	        		</p>
	        		<p>
	        			<span>C类：&nbsp;C1:<%=homeEntity.getC1Count()%>个</span>
	        			<span style="padding-left: 15px;">C2:<%=homeEntity.getC2Count()%>个</span>
	        			<span style="padding-left: 15px;">C3:<%=homeEntity.getC3Count()%>个</span>
	        			<span style="padding-left: 15px;">C4:<%=homeEntity.getC4Count()%>个</span>
	        			<span style="padding-left: 67px;">D类：&nbsp;D1:<%=homeEntity.getD1Count()%>个</span>
	        			<span style="padding-left: 15px;">D2:<%=homeEntity.getD2Count()%>个</span>
	        		</p>
			    </div>
        	</div>
        </div>
