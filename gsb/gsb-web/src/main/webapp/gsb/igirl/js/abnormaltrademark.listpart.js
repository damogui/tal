...
com.gongsibao.igirl.tm.web.TradeMarkListPart = org.netsharp.panda.commerce.ListPart.Extends({
	ctor : function() {
		this.base();
		this.filterMarkState();
	},
...
//			}
		...
		filterMarkState : function() {
			$('#markState').combobox({
				loadFilter: function(data){
					for(var i=1;i<data.length;){
						if(data[i].value!='6'&&data[i].value!='7'&&data[i].value!='13'&&data[i].value!='15'&&data[i].value!='17'&&data[i].value!='18'&&data[i].value!='20'&&data[i].value!='21'){
							data.splice(i,1); 
						}else{
							i++;
						}
