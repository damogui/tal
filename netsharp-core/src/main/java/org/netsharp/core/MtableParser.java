...
import org.netsharp.core.property.EncryptionProperty;
...
public class MtableParser {
	...
	private static void parseProperty(Mtable mtable, Field field) {
		...
		{
			Encryption cryptionMeta = (Encryption) field.getAnnotation(Encryption.class);

			if (cryptionMeta != null) {
				dataType = DataTypes.Encryption;
			}
		...
		if (dataType == DataTypes.Encryption) {
			property = new EncryptionProperty();
			{
