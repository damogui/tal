...
import org.netsharp.wx.mp.api.jssdk.JsApiTicketResponse;
...
public class WeixinCache {
    ...
    public AccessToken getToken(String appId) {
        ...
        if (cacheCommand.exists(realKey)) {
            token = (AccessToken) cacheCommand.get(realKey,AccessToken.class);
        }
        return token;
    }
    ...
    public JsApiTicketResponse getJsApiTicket(String appId) {
        ...
        if (getCacheCommand().exists(realKey)) {
            return (JsApiTicketResponse) getCacheCommand().get(realKey,JsApiTicketResponse.class);
        }
        return null;
    }
    ...
    private ICacheCommand getCacheCommand() {
        ...
        try {
            ...
            cacheCommand = CacheCommandFactory.create(CacheType.db);
        } finally {
            lock.unlock();
        }
        return cacheCommand;
