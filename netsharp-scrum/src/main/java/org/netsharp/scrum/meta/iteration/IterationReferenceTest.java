...
public class IterationReferenceTest {
	Class<?> type = Iteration.class;
	...
	private PDatagrid createDatagrid(ResourceNode node) {
		...
		{
			column.toNew();
			column.setPropertyName("isCurrent");
			column.setHeader("当前迭代");
			column.setControlType(ControlTypes.TEXT_BOX);
			column.setWidth(60);
		}
		datagrid.getColumns().add(column);
		...
		{
			column.toNew();
			column.setPropertyName("startTime");
			column.setHeader("起始日期");
			column.setControlType(ControlTypes.TEXT_BOX);
			column.setWidth(100);
		...
		{
			column.toNew();
			column.setPropertyName("endTime");
			column.setHeader("结束日期");
			column.setControlType(ControlTypes.TEXT_BOX);
			column.setWidth(100);
		}
		datagrid.getColumns().add(column);
