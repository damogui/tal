...
public class StoryFinishedWorkspaceTest extends StoryWorkspaceTest {
	...
	public void setup() {	
		...
		// 在子类中重定义
		urlList = "/scrum/my/finished/list";
		urlForm = "/scrum/my/finished/form";
		entity = Story.class;
		meta = MtableManager.getMtable(entity);
		listPartName =formPartName= "已完成任务";
		resourceNodeCode="my-scrum-finished";
		...
		listFilter = "status not in (1,3) and story.status != 10 and (owner_id = '{userId}' or id in (select story_id from "+tableName+" where participant_id = '{userId}'))";
	} 
