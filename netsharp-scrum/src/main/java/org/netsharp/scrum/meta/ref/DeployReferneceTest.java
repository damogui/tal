...
public class DeployReferneceTest {
	
	Class<?> type = Deploy.class;
	...
	private PDatagrid createDatagrid(ResourceNode node) {
		...
		{
			column.toNew();
			column.setPropertyName("deployTime");
			column.setHeader("部署日期");
			column.setControlType(ControlTypes.DATE_BOX);
			column.setFrozen(true);
			column.setWidth(120);
		}
		datagrid.getColumns().add(column);
