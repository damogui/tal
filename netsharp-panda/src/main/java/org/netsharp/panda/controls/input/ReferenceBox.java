...
import org.netsharp.authorization.PermissionManager;
...
public class ReferenceBox extends Input {
	...
	public void DataBind() {
		...
		this.url = "/panda/rest/reference?id=" + rd.getId() + "&filter=" + this.filter;
		...
		if (rd.isCanNew()) {
			...
			boolean isAddPermission = PermissionManager.isPermission(resourceNode, ot);
			if (isAddPermission) {
				...
				builder.append("$('#" + this.getId() + "').combogrid('hidePanel');");
				builder.append("IMessageBox.window('新增', '/panda/" + rd.getPopupUrl() + "?openType=window', " + rd.getWidth() + ", " + rd.getHeight() + ", function(){");
				builder.append("});");
				builder.append("}");
				builder.append("}]");
				this.getDataOptions().add(builder.toString());
