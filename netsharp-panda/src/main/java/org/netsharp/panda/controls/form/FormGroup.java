	...
	public void initialize() {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			for (int columnIndex = 0; columnIndex < columnCount; columnIndex++) {
				...
				if (formField.getControlType() == ControlTypes.CUSTOMER) {
					...
					if (control instanceof Input) {
						...
						if(StringManager.isNullOrEmpty(ci.controlType)){
							...
							ci.controlType = ControlTypes.CUSTOMER.name();
							ci.customerControlType = formField.getCustomerControlType();
						}
					}
				}
				...
				if (!StringManager.isNullOrEmpty(formField.getTooltip())) {
					...
					I icon = new I();
					{
						...
						icon.setClassName("easyui-tooltip fa fa-question-circle-o");
					}
					trc.getControls().add(icon);
				}
				index++;
			}
		}
