...
public class FormPartBase extends Part {
	...
	protected void addJscript() {
		...
		String title = Application.getContext().getName();
		if(!StringManager.isNullOrEmpty(this.context.getName())){
			title = this.context.getName();
		}
		this.addJscript("        document.title=\"" + title + "\";", JscriptType.Header);
