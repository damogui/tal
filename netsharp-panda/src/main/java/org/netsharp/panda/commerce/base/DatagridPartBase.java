			...
			if (dcolumn.getControlType() == ControlTypes.CHECK_BOX) {
				...
				column = new CheckBoxColumn();
				{
					...
					if(rowToolbar != null && rowToolbar.getControls().size() > 0){
						...
						int width = rowToolbar.controls.size() * 50;
...
//		if (!StringManager.isNullOrEmpty(formatter)) {
//
//			column.formatter = String.format("function(value,row,index){%s}", formatter);
//		}

//		String styler = dcolumn.getStyler();
//		if (!StringManager.isNullOrEmpty(styler)) {
//
//			column.styler = String.format("function(value,row,index){%s}", styler);
//		}
...
//		}
	...
	protected void addJscript() {
		...
		this.addJscript("        " + getJsInstance() + ".context.isTip=" + (!StringManager.isNullOrEmpty(this.context.getMemoto())) + ";",
				JscriptType.Header);
