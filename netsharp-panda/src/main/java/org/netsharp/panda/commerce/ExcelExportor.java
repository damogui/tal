...
public class ExcelExportor {
	...
	public void export(List<?> table) throws Exception {
		...
		String nameString = this.datagrid.getResourceNode().getName()
				.replace('-', '_')
				+ CodeUtil.generateTimedNo(20);
		String fileName = nameString + ".xls";
		String filePath = HttpContext.getCurrent().getRequest().getContextPath()+ "/download/panda-res/"+fileName;
		filePath = HttpContext.getCurrent().getContext().getRealPath(filePath);
		createExcel(table, filePath);
		downloadExcel(fileName, filePath);
	}
	...
	private void downloadExcel(String fileName, String filePathWithName)
			throws Exception {

		IResponse response = HttpContext.getCurrent().getResponse();
		ServletRequest request = (ServletRequest) HttpContext.getCurrent()
				.getRequest();
		response.reset();
		response.setContentType("application/vnd.ms-excel;charset=UTF-8");
		response.setHeader("Content-Disposition",
				"attachment;filename=" + this.getStr(request.request, fileName));
