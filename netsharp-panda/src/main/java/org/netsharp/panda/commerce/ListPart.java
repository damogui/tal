...
public class ListPart extends DatagridPartBase {
	...
	public Object query() throws IOException {
		...
		List<?> list = this.doQuery(oql);
		Object json = this.serialize(list, oql);
		return json;
	}
	...
	public void export() throws Exception {
		...
		if (!StringManager.isNullOrEmpty(filter)) {
			...
			filter = filter.replace('_', '=');
			filter = filter.replace('|', '=');
			System.out.println("export filter:" + filter);
		}
	...
	protected void renderDatagrid() {
		...
		datagrid.pageList = "[10,20,25,30,50,100,200]";
		...
		if (!StringManager.isNullOrEmpty(urlFilter)) {
			...
			try {
				...
				System.out.println("renderDatagrid urlFilter:" + urlFilter);
			} catch (UnsupportedEncodingException e) {
