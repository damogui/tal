...
public class FormPart extends FormPartBase {
	...
	public FormNavigation pre(Object id, Paging paging) {
		...
		if (obj == null) {
			fn.Entity = this.newInstance(null);
			System.out.println("id(" + id + ")前一张未查到实体!");
		} else {
			fn.Entity = obj;
		}
		fn.Paging = oql.getPaging();
	...
	public FormNavigation next(Object id, Paging paging) {
		...
		if (obj == null) {
			fn.Entity = this.newInstance(null);
			System.out.println("id(" + id + ")下一张未查到实体!");
		} else {
			fn.Entity = obj;
		}
		fn.Paging = oql.getPaging();
	...
	public FormNavigation last(Object id) {
		...
		if (obj == null) {
			fn.Entity = this.newInstance(null);
			System.out.println("id(" + id + ")最后一张未查到实体!");
		} else {
			fn.Entity = obj;
		}
		fn.Paging = oql.getPaging();
	...
	protected void importJs(IHtmlWriter writer) {
		...
		field = null;
		for (PFormField x : nform.getFields()) {
			if (x.getControlType() == ControlTypes.QINIUUPLOAD) {
				field = x;
				break;
			}
		}
		if (field != null) {
			if (Application.getContext().getEnvironment().equals(Environment.Dev)) {
