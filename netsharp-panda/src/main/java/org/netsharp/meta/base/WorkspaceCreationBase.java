...
public abstract class WorkspaceCreationBase {
	...
	protected void createListWorkspace() {
		...
		PWorkspace workspace = new PWorkspace();
		{
			...
			workspace.setName(listPartName+"_列表");
	 ...
	 */
	protected PDatagridColumn addColumn(PDatagrid datagrid, String propertyName, String header, ControlTypes controlType, int width,
			Boolean isFrozen, Integer referenceId, String groupName, OrderbyMode orderbyMode) {
		...
		if (controlType == ControlTypes.DATE_BOX) {
			...
			column.setFormatter(" if(value){return new Date(value).format('yyyy-M-dd');}");
			column.setWidth(80);
		...
		} else if (controlType == ControlTypes.DECIMAL_BOX || controlType == ControlTypes.CURRENCY_BOX || controlType == ControlTypes.NUMBER_BOX
				|| controlType == ControlTypes.PERCENTAGE_BOX) {
	 ...
	 * @throws
	 */
	protected PDatagridColumn addRefrenceColumn(PDatagrid datagrid, String propertyName, String header, Class<?> refrenceClazz, int width,
			Boolean isFrozen, Integer referenceId, String groupName, OrderbyMode orderbyMode) {
	...
	protected void createFormWorkspace() {
		...
		PWorkspace workspace = new PWorkspace();
		{
			workspace.toNew();
			workspace.setResourceNode(node);
			workspace.setName(formPartName+"_表单");
	 ...
	 * @throws
	 */
	protected PFormField addFormFieldRefrence(PForm form, String propertyName, String header, String groupName, String refrenceCode,
	 ...
	 * @throws
	 */
	protected PFormField addFormField(PForm form, String propertyName, String header, String tip, String groupName, ControlTypes controlType,
			Boolean isRequired, Boolean isReadOnly) {
	 ...
	 * @throws
	 */
	protected PToolbarItem addToolbarItem(PToolbar toolbar, String code, String name, String icon, String command, OperationType operationType,
			int seq) {
	...
	protected PRowStyler addRowStyler(PDatagrid datagrid, String rowCondition, String rowStyle) {
		...
		styler.toNew();
		styler.setRowCondition("row.disabled");
