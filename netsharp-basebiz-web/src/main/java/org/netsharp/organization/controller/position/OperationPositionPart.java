package org.netsharp.organization.controller.position;
...
import org.netsharp.organization.controller.authorization.operation.AuthorizationSerializer;
import org.netsharp.organization.controller.authorization.operation.AuthorizationStruct;
import org.netsharp.organization.controller.authorization.operation.OperationCategoryStruct;
...
public class OperationPositionPart extends ListPart {
...
//		datagrid.Url = "/panda/rest/service?vid=" + this.context.getId() + "&method=query";
	...
	private void renderTreegrid() {
		this.treegridSchemaData();
		this.treegridSchema();
	}

	private void treegridSchemaData() {
		// 查询功能资源树
		resourceNodes = operationService.queryOperationResourceNodes(false);

		// 查询可勾选的功能权限
		operations = operationService.querySettingOperations(false);
	...
	// / 更新某个操作的权限
	// / </summary>
	public void updatePrincipalOperation(String idPrincipal,PrincipalType principalType, String operationIds, String operationType) {
		...
		UiOperationType uiType = UiOperationType.valueOf(operationType);

		IPrincipalOperationService service = ServiceFactory.create(IPrincipalOperationService.class);

		service.updatePrincipalOperation(principalId,principalType, operationIds, uiType);
		
		// 更新当前用户的Session ,应该是更新所有的Session，目前只更新当前用户
		UserPermissionManager.resetUserPermission();
	}
}
