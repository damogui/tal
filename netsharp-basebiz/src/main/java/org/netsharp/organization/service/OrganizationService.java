...
public class OrganizationService extends PersistableService<Organization> implements IOrganizationService {
		...
		if (filter != null && filter.indexOf("resource.id") > -1) {
			...
			for (int i = 0, len = strs.length; i < len; i++) {
				if (strs[i].indexOf("resource.id") > -1) {
					...
					strs[i] = "(EXISTS (SELECT 1 FROM sys_permission_principal_operation po LEFT JOIN sys_permission_operation op ON po.operation_id=op.id WHERE Organization.id=po.principal_id AND op.resource_node_id=" + ss[1] + "))";
					break;
				}
			}
	...
	public List<Organization> getDirectDepartmentByEmployeeId(Integer employeeId) {
		...
		Oql oql = new Oql();
		{
			...
			oql.setFilter("organizationType = 'Department' and pathCode in ( " + StrUtil.join(pathCodes, ",") + ")");
		}
	...
	protected Employee getEmployeeByPathCodeAndPositionId(String pathCode, String positionId) {
		Oql oql = new Oql();
		{
			...
			oql.setOrderby("Id desc");
			oql.setFilter("  organizationType = 'Post'  and   path_code   like  '" + pathCode + "%'   and  positionId='" + positionId + "'");
		}
	...
	public Organization getCorprationByDepartment(String deptPathCode) {
		...
		Oql oql = new Oql();
		{
			...
			oql.setSelects("Organization.*");
			oql.setFilter(" OrganizationType = '" + OrganizationType.CORPORATION + "'   or  OrganizationType ='" + OrganizationType.SYSTEM + "'");
	...
	@Override
	public Employee getDirectLeaderByDepartmentId(Integer deptId) {
		// TODO Auto-generated method stub
		// 获取员工的部门
		Oql oql = new Oql();
		{
		...
		oql = new Oql();
		{
			...
			oql.setFilter("  organizationType = 'Department'  and  POSITION(path_code IN '" + pathCode + "')=1  ");
		...
		oql = new Oql();
		{
			...
			oql.setFilter("  organizationType = 'Post'  and   path_code   like  '" + deptRootPathCode + "%'   and  positionId='" + postionId + "'");
		}
