...
public class EmployeeService extends PersistableService<Employee> implements IEmployeeService {
	...
	public Employee byId(Object id) {
		...
		Oql oql = new Oql();
		{
			...
			oql.setSelects("employee.*,employee.organizations.*,employee.organizations.organization.*,employee.citys.*,employee.citys.fProvince.*,employee.citys.fCity.*,employee.citys.fCounty.*");
			oql.setFilter(" id=? ");
	...
	public boolean resetPassword(Integer id) {
		...
		if (employee != null) {
			...
			String pwd = EncrypUtil.md5(Application.getContext().getDefaultPassword() + "user!@#123").substring(8,24);
