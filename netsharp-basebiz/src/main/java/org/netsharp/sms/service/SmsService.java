...
public class SmsService implements ISmsService {
	...
	public void sendVoice(String mobile, String content, String type) {
		// 非开发环境，不发送短信，查看短信日志即可
		if (Application.getContext().getEnvironment() == Environment.Product || Application.getContext().getEnvironment() == Environment.Dev) {
			for (String exclusive : exclusives) {
				if (exclusive.equals(mobile)) {
					return;
				}
			...
			logger.info("只有product,test服务器才可以发送短信，当前环境是：" + Application.getContext().getEnvironment());
		}
